...
As a user I want to be able to update the information of a todo task that was added to the todo list
  Background:
    ...
    Scenario: Update an existing todo instance with a malformed JSON request body
        When I send a POST request to the API for a todo with an existing id, a new valid "<title>" value, a valid "<doneStatus>" value and a valid "<description>" value in the JSON request body but with a missing comma
